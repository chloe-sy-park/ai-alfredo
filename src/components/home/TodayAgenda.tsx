...
import { Check, Pencil, Star, Clock, Plus, X, Bot, ChevronDown, ChevronUp } from 'lucide-react';
import { Task, getTasks, updateTask } from '../../services/tasks';
...
interface AgendaItem extends Task {
  subtasks?: Subtask[];
  progress?: number;
}
...
export default function TodayAgenda({ onCompleteTask }: TodayAgendaProps) {
  ...
  const loadAgendaItems = () => {
    // 오늘 날짜의 Task 또는 starred Task 가져오기
    ...
    setAgendaItems(items.slice(0, 3)); // 최대 3개
  };
  ...
  if (agendaItems.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
        <h2 className="font-bold text-gray-900 dark:text-white mb-3">Today's Agenda</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">
          오늘의 주요 일정이 없습니다.<br />
          Task에 별표를 추가하거나 오늘 예정일을 설정하세요.
        </p>
      </div>
    );
  }
    ...
    <>
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-card">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 dark:bg-gray-700 rounded-full text-xs font-medium text-primary hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
            <Bot size={14} />
            AI MENTOR CARE
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-t-3xl p-5 pb-8 animate-slideUp safe-area-bottom max-h-[85vh] overflow-y-auto">
            ...
            <div className="mb-4">
              ...
              {editingItem.subtasks && editingItem.subtasks.length > 0 && (
                <div className="space-y-2 mb-3">
                  {editingItem.subtasks.map((subtask) => (
                    <div
                      key={subtask.id}
            ...
            <button
              ...
              className="w-full py-3 bg-gray-100 dark:bg-gray-700 rounded-xl text-sm font-medium text-primary hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center justify-center gap-2"
            >
              <Bot size={16} />
              AI가 서브태스크 제안
            </button>
  ...
  return (
    <div className="border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden">
      ...
      <div className="grid grid-cols-2 border-t border-gray-100 dark:border-gray-700">
        ...
        <button
          onClick={onEdit}
          className="flex items-center justify-center gap-2 py-3 text-sm font-medium text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-l border-gray-100 dark:border-gray-700"
        >
          <Pencil size={16} />
          EDIT
        </button>
